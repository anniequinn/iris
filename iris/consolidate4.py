...
def handle_colour_collection(func):
    def wrapper(colour_input, target_colour_space):
        ...
        if isinstance(colour_input, ColourCollection):
            converted_colours = [
                func(colour, target_colour_space)
                for colour in colour_input.colours
            ]
            return ColourCollection(converted_colours)

        elif isinstance(colour_input, list):
            return [
                (
                    func(MyClass(colour), target_colour_space)
                    if not isinstance(colour, MyClass)
                    else func(colour, target_colour_space)
                )
                for colour in colour_input
            ]
        else:
...
def convert_colour(colour_input, target_colour_space: str) -> MyClass:
    ...
    if path_key not in conversion_paths:
        raise ValueError(
            f"No conversion path from {source_colour_space} to {target_colour_space}"
        )
...
convert_colour(col_collection, "lab")