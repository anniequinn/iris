...
def handle_colour_collection(func):
    def wrapper(colour_input, target_colour_space):
        # Check if the input is a list of colour inputs
        if isinstance(colour_input, list):
            return [func(MyClass(colour), target_colour_space) if not isinstance(colour, MyClass) else func(colour, target_colour_space) for colour in colour_input]
        else:
            # Process a single colour input
            return func(colour_input, target_colour_space)
    return wrapper


@handle_colour_collection
def convert_colour(colour_input, target_colour_space: str) -> MyClass:
    if isinstance(colour_input, MyClass):
        colour_instance = colour_input
    else:
        try:
            colour_instance = MyClass(colour_input)
        except ValueError as e:
            raise ValueError(f"Could not convert input to MyClass: {e}")
    
    colour = colour_instance.iris_colour
    source_colour_space = colour_instance.iris_colour_space

    conversion_paths = {
        ("hex", "rgb"): [Converter.hex2rgb],
        ("hex", "xyz"): [Converter.hex2rgb, Converter.rgb2xyz],
        ("hex", "lab"): [Converter.hex2rgb, Converter.rgb2xyz, Converter.xyz2lab],

        ("rgb", "hex"): [Converter.rgb2hex],
        ("rgb", "xyz"): [Converter.rgb2xyz],
        ("rgb", "lab"): [Converter.rgb2xyz, Converter.xyz2lab],

        ("xyz", "hex"): [Converter.xyz2rgb, Converter.rgb2hex],
        ("xyz", "rgb"): [Converter.xyz2rgb],
        ("xyz", "lab"): [Converter.xyz2lab],

        ("lab", "hex"): [Converter.lab2xyz, Converter.xyz2rgb, Converter.rgb2hex],
        ("lab", "rgb"): [Converter.lab2xyz, Converter.xyz2rgb],
        ("lab", "xyz"): [Converter.lab2xyz],
    }

    path_key = (source_colour_space, target_colour_space)
    if path_key not in conversion_paths:
        raise ValueError(f"No conversion path from {source_colour_space} to {target_colour_space}")
    
    for conversion_step in conversion_paths[path_key]:
        colour = conversion_step(colour)

    return MyClass(colour, target_colour_space)
...
convert_colour(nice_colour, "lab")

convert_colour("rgb(255,0,0)", "hex")

convert_colour(["#FF0000", "#00FF00", "rgb(0,255,0)"], "lab")
